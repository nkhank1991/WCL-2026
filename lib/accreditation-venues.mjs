...
export function seatingError(config, proposal, {required = true} = {}) {
  ...
  // New approvals use numbered sections, not a ticket seating map. Existing
  // allocated credentials still retain and enforce every frozen seating limit.
  if(proposal.accessMode==='sections' && !selected.length)return '';
...
export function seatingScanError(config, snapshot, venue, zone, area) {
  if(snapshot?.accessMode==='sections' && !snapshot.seatingAreas?.length)
    return area ? 'This credential uses section access, not a seating checkpoint' : '';
