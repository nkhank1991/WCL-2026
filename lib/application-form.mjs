...
export const originalIdAtCollection=config=>config?.identityVerificationMode==='original-at-collection';
export const identityCopyFields=[...documentKinds,'idType','idDescription','idHasReverse','idNumber','passportNumber','emiratesIdNumber'];
export const hasIdentityCopyFields=body=>Object.keys(body?.documents||{}).length>0||identityCopyFields.some(k=>body?.[k]!==undefined&&body[k]!==''&&body[k]!==false);
...
export function applicationFieldError(config,body){
  ...
  if(!originalIdAtCollection(config)&&!(config.simpleApplication?['passport','emirates-id']:['passport','emirates-id','other']).includes(body.idType))return 'Choose your passport or Emirates ID.';
  if(!originalIdAtCollection(config)&&body.idType==='other'&&!String(body.idDescription||'').trim())return 'Specify the government photo ID you are providing.';
  ...
  if(!config.simpleApplication&&d.nominationRequired&&(!String(body.nominatorName||'').trim()||!/^([^\s@]+@[^\s@]+\.[^\s@]+|\+[0-9 ()-]{7,20})$/.test(String(body.nominatorContact||''))))return 'Add your nominating manager or department contact and their email or international phone number.';
