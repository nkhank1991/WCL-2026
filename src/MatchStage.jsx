...
export function MatchInsights({match,players=[],videos=[]}){
 ...
 const [selected,setSelected]=useState('information'),id=useId();
 useEffect(()=>setSelected('information'),[match.id]);
 ...
 const tabs=[{id:'information',label:'Overview'},...(live?.innings.length||match.scoreSource?[{id:'scorecard',label:'Scorecard'}]:[]),...(live?.recentBalls.length?[{id:'commentary',label:'Commentary'}]:[]),...(people.length?[{id:'statistics',label:'Statistics'}]:[]),...(videos.length?[{id:'highlights',label:'Highlights'}]:[])];
 const active=tabs.some(tab=>tab.id===selected)?selected:'information';
 function tabKey(event){const keys=['ArrowLeft','ArrowRight','Home','End'];if(!keys.includes(event.key))return;event.preventDefault();const buttons=[...event.currentTarget.parentElement.querySelectorAll('[role=tab]')],index=buttons.indexOf(event.currentTarget);const next=event.key==='Home'?0:event.key==='End'?buttons.length-1:(index+(event.key==='ArrowRight'?1:-1)+buttons.length)%buttons.length;setSelected(tabs[next].id);buttons[next].focus();}
 const panelProps=key=>({id:id+'-'+key,role:'tabpanel','aria-labelledby':id+'-tab-'+key,hidden:active!==key,tabIndex:0});
 return <div className="ms-insights">
  <div className="ms-detail-tabs" role="tablist" aria-label="Match details">{tabs.map(tab=><button key={tab.id} id={id+'-tab-'+tab.id} role="tab" aria-selected={active===tab.id} aria-controls={id+'-'+tab.id} tabIndex={active===tab.id?0:-1} onKeyDown={tabKey} onClick={()=>setSelected(tab.id)}>{tab.label}</button>)}</div>
  <section {...panelProps('information')}><dl className="ms-match-information"><div><dt>Competition</dt><dd>WCL · Season {match.season}</dd></div><div><dt>When</dt><dd>{matchMoment(match)}</dd></div><div><dt>Venue</dt><dd>{venueLabel(match)}</dd></div><div><dt>Status</dt><dd>{state}</dd></div></dl><p className="ms-availability-note">{state==='Scheduled'?'Live updates not yet available.':state==='Archive'?'Published fixture archive. No verified result available.':!live&&!match.scoreSource?'Detailed match data is not connected.':null}</p></section>
  {(live?.innings.length>0||match.scoreSource)&&<section {...panelProps('scorecard')}>{live?.innings.length>0&&<div className="ms-innings">{live.innings.map(innings=><div key={innings.teamId}><strong>{teamName(innings.teamId)}</strong><span>{innings.runs}/{innings.wickets}</span><small>{innings.overs} overs</small></div>)}</div>}{match.scoreSource&&<a className="ms-score-source" href={match.scoreSource} target="_blank" rel="noopener noreferrer">Full scorecard on Cricbuzz <ArrowRight/></a>}</section>}
  {live?.recentBalls.length>0&&<section {...panelProps('commentary')}><h2 className="ms-panel-title">Recent deliveries</h2><ol className="ms-commentary">{live.recentBalls.map(ball=><li key={ball.id}><strong>{ball.over}</strong><span>{ball.summary||ball.label}</span></li>)}</ol></section>}
  {people.length>0&&<section {...panelProps('statistics')} className="ms-players"><div className="ms-section-heading"><h2>Player statistics</h2><span>{state==='Result'?'Match performances':'Current innings'}</span></div><div className="ms-player-line">{people.map(({stats,player})=><Link key={player.id} to={'/players/'+player.id} className="ms-player"><div className="ms-player-photo">{player.image&&<img src={deliveryImage(player.image)} alt={player.name} loading="lazy"/>}</div><div><small>{teamName(player.team)} · {stats.role==='batting'?'Batting':'Bowling'}</small><h3>{player.name}</h3><p>{stats.role==='batting'?<><strong>{stats.runs}{stats.striker?'*':''}</strong><span>from {stats.balls} balls</span></>:<><strong>{stats.wickets}/{stats.conceded}</strong><span>{stats.overs} overs</span></>}</p></div><ArrowRight/></Link>)}</div></section>}
  {videos.length>0&&<section {...panelProps('highlights')}><div className="ms-highlight-links">{videos.map(video=><Link to={'/watch/'+video.id} key={video.id}>{video.thumbnail&&<img src={video.thumbnail} alt="" loading="lazy"/>}<span>{video.title}</span><ArrowRight/></Link>)}</div></section>}
 </div>;
}
