...
import {pageMetadata,directoryGroups} from './model.js';
...
export function CricketAnswers(){
  return <><div className="page-title"><div className="wrap"><p className="kicker">GOOD TO KNOW</p><h1>Frequently asked questions</h1><p>Clear answers. Your next step, one click away.</p></div></div><section className="wrap section faq-list">{answers.map(({q,a,links})=><details key={q}><summary>{q}</summary><p>{a}</p><div className="answer-links">{links.map(([label,to])=><Link key={to} to={to}>{label} <span aria-hidden="true">↗</span></Link>)}</div></details>)}</section></>;
}
export function SiteDirectory(){
  return <><div className="page-title"><div className="wrap"><p className="kicker">EXPLORE WCL</p><h1>Every team. Every chapter.</h1><p>Teams, players, fixtures and stories — all in one place.</p></div></div><section className="wrap section site-directory">{directoryGroups().map(group=><details key={group.title} open><summary>{group.title}<span>{group.items.length}</span></summary><ul>{group.items.map(item=><li key={item.path}><Link to={item.path}>{item.label}</Link></li>)}</ul></details>)}</section></>;
}
