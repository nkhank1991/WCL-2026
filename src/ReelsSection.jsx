...
export function ReelsSection(){
 const instagram=usePublished('social',socialDefaults).find(l=>l.platform==='instagram'&&!l.team&&l.visible!==false)?.url;
 const news=usePublished('news',source),{enabled}=useFilmMotion();
 const curated=usePublished('reels',socialReels);
 const items=[...curated,...selection.map(id=>news.find(n=>n.id===id))].filter(n=>n?.thumbnail&&instagramSource(n.sourceUrl));
 ...
 return !items.length?null:<section className="reels-section wrap section" aria-labelledby="reels-title">
  <div className="section-title"><div><p className="kicker broadcast-kicker"><SectionRule/>INSIDE WCL</p><h2 id="reels-title">Closer to the moment.</h2></div>{instagram&&<a className="text-link" href={instagram} target="_blank" rel="noopener noreferrer"><InstagramLogo/> Follow WCL <ArrowUpRight/></a>}</div>
