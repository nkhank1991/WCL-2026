...
import {requestedTeamRole,originalIdAtCollection} from '../../lib/application-form.mjs';
...
export function SimpleApplicationFields({config,busy,onPhoto}){
  ...
  return <>
    ...
    <ApplicationSection number="02" title={noCopy?"Your photograph":"Photo & ID"}>
      <PortraitCrop onChange={onPhoto}/>
      {noCopy?<p className="ops-caption">Upload a portrait only—not an ID document. Bring your original Emirates ID or passport when collecting your badge. We do not keep an ID copy or number.</p>:<><label>ID type<select name="idType" required><option value="">Select ID</option><option value="emirates-id">Emirates ID</option><option value="passport">Passport</option></select></label>
    ...
    <ApplicationSection number="03" title="Requested access">
      ...
      <label className="ops-check"><input type="checkbox" name="accuracy" required/>I confirm that these details and the photograph are accurate and I am authorised to submit them.</label>
