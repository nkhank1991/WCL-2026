...
import {policyDocuments,policyPaths,policyDate,policyVersion,draftNotice,policyReferences} from './policy-content';
...
export function AccreditationPolicyPage({kind='privacy'}){
  ...
  return <div className="ops-shell accreditation-policy">
    ...
    <main>
      ...
      <aside className="policy-related"><h2>Read alongside this draft</h2>{Object.entries(policyPaths).filter(([key])=>key!==kind).map(([key,to])=><Link key={key} to={to}>{policyDocuments[key].title}</Link>)}<Link to="/privacy">Website privacy policy</Link></aside>
      <aside className="policy-related"><h2>Research references</h2><p>Official sources reviewed on {policyDate}. These inform the draft; they do not certify WCL compliance or approve its provider arrangements.</p>{policyReferences.map(([label,url])=><a key={url} href={url} target="_blank" rel="noreferrer">{label}</a>)}</aside>
    </main>
    <footer className="ops-footer"><span>WCL accreditation · draft policy</span><Link to="/accreditation/apply">Return to application</Link></footer>
  </div>;
}
